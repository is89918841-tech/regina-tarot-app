...
router.post('/consultations/:id/reading/generate', async (req, res, next) => {
  try {
    ...
    const isStandardMenu =
      menuText.includes('7,000') ||
      menuText.includes('7000') ||
      menuText.includes('핵심 정리') ||
      menuText.includes('핵심정리');

    const isReservedMenu =
      menuText.includes('18,000') ||
      menuText.includes('18000') ||
      menuText.includes('30,000') ||
      menuText.includes('30000') ||
      menuText.includes('결정 흐름') ||
      menuText.includes('심층 결정');

    if (isReservedMenu) {
      return res.json({
        ok: false,
        reserved: true,
        error: '예약형 리딩은 자동 생성하지 않습니다. 직접 리딩해주세요.',
      });
    }
...
- 450~650자 내외
- 첫 문장에서 결론을 바로 말할 것
- 현재 상태, 핵심 이유, 조언, 최종 결론을 한 문단 안에 모두 포함할 것
- 깊은 설명보다 “지금 어떻게 판단해야 하는지”를 우선할 것
`
      : `
[7,000원 리딩 규칙]
- 3~4문단으로 작성
- 첫 문단 첫 문장에서 결론을 바로 말할 것
- 1문단: 결론 + 현재 상태
- 2문단: 왜 이런 흐름인지 이유 설명
- 3문단: 앞으로의 흐름
- 4문단: 지금 해야 할 행동 조언 + 최종 결론
- 추상적인 조언 금지
- “소통하세요”, “진정성이 중요해요” 같은 일반론 금지
- 반드시 실행 가능한 행동으로 정리할 것
`;
...
[리딩 스타일 핵심]
- 카드 이름 직접 언급 금지
- 카드의 숫자, 상징, 아르카나 표현 직접 언급 금지
- 감정 + 현실 흐름을 함께 설명
- 위로보다 현실 판단과 결정 정리를 우선할 것
- 읽고 나면 “그래서 어떻게 해야 하는지”가 분명해야 함

[결론 시작 강제 규칙]
- 첫 문장은 반드시 전체 결론으로 시작할 것
- 돌려 말하지 말고 바로 판단부터 말할 것
- 첫 문장에는 반드시 아래 의미 중 하나가 들어가야 한다:
  1. 지금은 가는 흐름이다
  2. 지금은 멈추는 흐름이다
  3. 조건을 맞추면 가능하다
  4. 속도를 늦춰야 한다
- “가능성이 있어요”, “좋을 것 같아요”, “나쁘지 않아요”로 시작 금지
...
- “상황을 지켜보세요”로 끝내기 금지
- 반드시 확정형 문장 사용
- 단, 현실적으로 조건이 필요한 경우에는 “조건부 가능”으로 명확히 말할 것

[행동 구체화 규칙]
- 조언은 반드시 실행 가능한 행동으로 제시할 것
- 추상적인 표현 금지
- 예: “소통하세요” 금지
- 예: “이번 주 안에 가격표/후기/첫 문구 중 하나를 고쳐야 해요”처럼 구체화
...
[최종 문장 강제 규칙]
- 마지막 문장은 반드시 아래 4개 중 하나를 그대로 사용할 것
- 단어 변경, 조사 변경, 문장 변형 금지
