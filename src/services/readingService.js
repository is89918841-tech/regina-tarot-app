...
function detectIntent(question = '') {
  ...
  if (q.includes('연애') || q.includes('사랑') || q.includes('속마음') || q.includes('관계')) return '연애';
  if (q.includes('이직') || q.includes('취업') || q.includes('직업')) return '이직';
  if (q.includes('돈') || q.includes('재물') || q.includes('금전')) return '재물';
  if (q.includes('건강')) return '건강';
  if (q.includes('언제') || q.includes('시기')) return '시기';
  if (q.includes('어디') || q.includes('장소')) return '장소';
  if (q.includes('결정') || q.includes('선택')) return '결정';
...
async function generateReading({ question, spread, deck, topic }) {
 ...
 const cleanedReading = rawReading
  ...
  return {
    intent,
    knowledge,
    reading: normalizeReading(cleanedReading),
  };
}
