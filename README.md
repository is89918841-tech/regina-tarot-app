# regina-tarot-app[regina_reading_app_mobile_0816.html](https://github.com/user-attachments/files/21813333/regina_reading_app_mobile_0816.html)
<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
  <title>레지나 리딩 보조앱 (모바일 단일 파일)</title>
  <!-- Tailwind (CDN) -->
  ...
  <style>
    ...
    body { background: #0f172a; } /* slate-900 fallback before Tailwind paints */
  </style>
</head>
<body>
  <div id="root"></div>
  ...
  <script type="text/babel">
    ...
    const NAJEON_P = ["천성 별전","태극 별전","연화 별전","쌍벌 별전","오화형 별전","박쥐 별전","가지 전","모전 다듬기","식물 별전","분재 열쇄패"];
    const NAJEON_C = ["매조죽문항아리","당초문탁잔","국화문화형탁잔","국화모란장경병","청화국화문병","난초청낭자문병","도자기 다종","팔각통형병","표주박형병","십장생문각병"];
    const NAJEON_W = ["연화목 촛대","나비형목촛대","좌독기","혼례","과거시험","금의환향","투호","활","성곽기","투각파초문필통"];
    const NAJEON_S = ["운검","월도","검무","금제감장보검","환도빼앗기","실어 보내는 검","못 가져간 인검","모란각인환도","베갯머리 인검","침상 인검"];
    const NAJEON_COURTS = {
    ...
    const ROSEDIC_78 = [...TAROT_MAJOR, ...buildRosedicMinors()];
    ...
    function getDeckCards(deckName){
      if(deckName === '나전의 빛 타로') return NAJEON_78;
      if(deckName === '로제딕 타로') return ROSEDIC_78;
      if(deckName === '유니버셜 타로' || !deckName) return TAROT_78;
      return TAROT_78;
    }

    function drawRandomTarot(deckCards, count, allowReversal=true, reverseRatio=50){
      const deck = shuffleInPlace([...deckCards]);
      return deck.slice(0,count).map(name=>({ name, reversed: !!allowReversal && Math.random()*100 < reverseRatio }));
    }
    const drawRunes = (n)=> shuffleInPlace([...RUNES_24]).slice(0,n);
    const drawLenormand = (n)=> shuffleInPlace([...LENORMAND_36]).slice(0,n);
    const drawOgangi = ()=> shuffleInPlace([...OGANGI_5])[0];
    const drawObanggi = ()=> shuffleInPlace([...OBANGGI_5])[0];
    ...
    function setOracleDeckCards(deckName, names){ const list = (names||[]).filter(v=>typeof v==='string' && v.trim()).map(v=>v.trim()); ORACLE_CARDS_BY_DECK = { ...ORACLE_CARDS_BY_DECK, [deckName]: list }; saveOracleCardsMap(ORACLE_CARDS_BY_DECK); }
    function getOracleDeckCards(deckName){ if (PRESET_ORACLE_CARDS[deckName]) return PRESET_ORACLE_CARDS[deckName]; const list = ORACLE_CARDS_BY_DECK[deckName]; return Array.isArray(list) && list.length>0 ? list : null; }
    ...
    // Pure formatter
    function buildTemplateText(r){
      if (!r) return '';
      const { tarotDeck, cardCount, tarot, runes, oracles, lenormand, aux } = r;
      const positions = tarot.map(t=>\`- \${t.position}: \${t.card}\`).join('\\n');
      const runesTxt = (runes?.length? \`\\n**룬**: \${runes.join(', ')}\`:'');
      const oracleTxt = (oracles?.length? \`\\n**오라클**: \${oracles.join(', ')}\`:'');
      const lenoTxt = (lenormand?.length? \`\\n**레노먼드**: \${lenormand.join(', ')}\`:'');
      const auxTxt = (aux?.length? \`\\n**보조**: \${aux.join(', ')}\`:'');
    ...
    function ReginaReadingApp(){
      const [tarotDeck,setTarotDeck]=React.useState(REGINA_TAROT_DECKS[0]);
      ...
      const [reverseRatio,setReverseRatio]=React.useState(50);
      const [cardCount,setCardCount]=React.useState(1);

      const [mode,setMode]=React.useState('random');
      const [numMap,setNumMap]=React.useState([]);
      const [selectedNums,setSelectedNums]=React.useState([]);

      const [useRunesState,setUseRunesState]=React.useState(false);
      const [runeCount,setRuneCount] = React.useState(2);
      const [useOracle,setUseOracle]=React.useState(false);
      const [oracleDeck,setOracleDeck]=React.useState(REGINA_ORACLE_DECKS[0]);
      const [oracleCount,setOracleCount]=React.useState(1);
      const [useLenormand,setUseLenormand]=React.useState(false);
      const [lenormandCount,setLenormandCount]=React.useState(2);
      const [useOgangi,setUseOgangi]=React.useState(false);
      const [useObanggi,setUseObanggi]=React.useState(false);

      const [result,setResult]=React.useState(null);
      const [copyStatus,setCopyStatus]=React.useState({status:'idle',message:''});
      const hiddenTaRef = React.useRef(null);
      const preRef = React.useRef(null);
      const [envInfo] = React.useState(()=>({ inIframe: (function(){try{return window.self !== window.top;}catch{return true;}})(), secure: (function(){try{return !!(window.isSecureContext);}catch{return false;}})() }));
      ...
      const [compact,setCompact] = React.useState(()=> detectCompact());
      React.useEffect(()=>{
      ...
      React.useEffect(()=>{ if(selectedNums.length>cardCount){ setSelectedNums(prev=>prev.slice(0,cardCount)); } },[cardCount, selectedNums.length]);
                ...
                {mode==='numbers' && (
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <button onClick={()=>{setNumMap(prev=>shuffleInPlace([...deckCardsMemo])); setSelectedNums([]);}} className="px-3 py-1.5 rounded bg-slate-700 hover:bg-slate-600 text-sm">맵 다시 섞기</button>
